...
using CuoiKy_Video4_LoginLogout.Models;

namespace CuoiKy_Video4_LoginLogout.Controllers
{
    public class LoginAccController : Controller
    {
        private QLDangNhapContext db = new QLDangNhapContext();
        ...
        {
            var user = db.Accouts.Where(elem => elem.username == username && elem.password == password).FirstOrDefault();
            if (user == null)
            {
